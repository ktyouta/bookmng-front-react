//YouTube Data Api(書籍詳細)のレスポンス
export type YouTubeDataApiBookDetailItemType = {
    readonly kind: string;
    readonly etag: string;
    readonly id: string;
    // 書籍の基本情報（タイトル、説明、サムネイル、チャンネル情報等）
    readonly snippet: {
    ...
    // 書籍の統計データ（再生回数、いいね数、コメント数等）
    readonly statistics?: {
        readonly viewCount: string;
        readonly likeCount?: string;
        readonly dislikeCount?: string;
        readonly bookshelfCount?: string;
        readonly commentCount?: string;
    };
    // 書籍の詳細情報（再生時間、解像度、字幕の有無等）
